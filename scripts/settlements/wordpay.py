...
def generate_worldpay_settlement(filename, num_rows, num_merchants):
    ...
    payment_types = ["VI", "MC", "AX", "DI", "eCheck"] # Visa, Mastercard, Amex, Discover
    ...
    for _ in range(num_rows):
        ...
        settled_deposits = Decimal(random.uniform(1000.0, 50000.0)).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        
        # Negative outbound/deduction amounts (Represented as negative decimals for balancing)
        # Refunds approx 5% of volume
        settled_refunds = -(settled_deposits * Decimal(random.uniform(0.01, 0.05))).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        
        # Chargebacks approx 1% of volume
        chargebacks = -(settled_deposits * Decimal(random.uniform(0.00, 0.01))).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        
        # Vantiv (Worldpay) processing fees approx 1.2%
        vantiv_fees = -(settled_deposits * Decimal("0.012")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        
        # Network Passthrough fees approx 0.8%
        passthrough_fees = -(settled_deposits * Decimal("0.008")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        
        # Occasional Reserve withholdings
        reserve_activity = Decimal("0.00")
        if random.random() < 0.10: 
            reserve_activity = -(settled_deposits * Decimal("0.05")).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        ...
        net_settlement = (settled_deposits + settled_refunds + chargebacks + 
                          vantiv_fees + passthrough_fees + reserve_activity + third_party_payments)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--file', type=str, default='Financial_Summary_SettlementReport_0_12345_20260305.csv')
